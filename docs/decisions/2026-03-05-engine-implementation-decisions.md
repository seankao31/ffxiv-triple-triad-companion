...
**Ascension/Descension stat snapshot (i++ timing):** `placeCard` counts all card types on `state.board` *before* placing the new card into a `Map<CardType, number>` (TS) / `HashMap<CardType, u8>` (Rust). This snapshot is used for the entire resolution (Plus, Same, Combo cascade, standard capture). The placed card never counts toward its own resolution — the count only takes effect for future turns. This mirrors the in-game rule where the newly placed card's type contribution applies after its captures resolve.

**Ascension/Descension scope:** Every card (attacker and defender) looks up its *own* type's count in the snapshot. Ascension boosts a card's edge values by the count of same-type cards already on the board; Descension penalizes by the same. This applies to all card types — not just Primal or Scion. Values are capped at 10 for Ascension and floored at 1 for Descension. Since the defender was placed in a prior turn, it is already on the board when the snapshot is taken and counts toward its own type's total.
